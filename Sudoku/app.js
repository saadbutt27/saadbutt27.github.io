...
window.addEventListener('keydown', (e) => {
    ...
    if(currentKey === "ArrowUp"){
        ...
        if(nextCell > -1){
            selectDiv(cells[nextCell],nextCell);
        }
        ...
        inputGiven(Number(currentKey-1));
    } else if(currentKey === "Backspace" || currentKey === "Delete"){
        removeInput(cells[selected_cell]);
    }
})
...
const initCellsEvent = () => {
    cells.forEach((e, index) => {
        e.addEventListener('click', () => selectDiv(e,index));
    });
};
const startGame = () => {
...
(_j = document.querySelector('#btn-delete')) === null || _j === void 0 ? void 0 : _j.addEventListener('click',() => removeInput(cells[selected_cell]));
