...
window.addEventListener('keydown', (e) => {
    ...
    if(currentKey === "ArrowUp"){
        let nextCell = selected_cell-9;
        if(nextCell > -1){
            selectDiv(<HTMLDivElement>cells[nextCell],nextCell);
        }
        ...
        let nextCell = selected_cell-1;
        if(selected_cell%9 != 0){
            selectDiv(<HTMLDivElement>cells[nextCell],nextCell);
        }
    ...
    } else if(+currentKey < 10 && +currentKey > 0) {
        inputGiven(+currentKey-1);
    } else if(currentKey === "Backspace" || currentKey === "Delete"){
        removeInput(<HTMLDivElement>cells[selected_cell]);
    }
})
...
function inputGiven(index: number) {
...
const initNumberInputEvent = () => {
    number_inputs.forEach((e, index) => {
        e.addEventListener('click', () => inputGiven(index))
    })
}

function selectDiv(e:HTMLDivElement, index: number) {
...
const initCellsEvent = () => {
    cells.forEach((e, index) => {
        e.addEventListener('click', () => selectDiv(<HTMLDivElement>e, index))
    })
}
...
document.querySelector('#btn-delete')?.addEventListener('click', () => removeInput(<HTMLDivElement>cells[selected_cell]))
// -------------

function removeInput (e:HTMLDivElement)  {
